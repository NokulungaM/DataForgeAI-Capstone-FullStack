        ...
        {recipes.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 justify-items-center">
            {recipes.map((recipe, index) => (
              <div
                key={recipe.id || index}
                className="bg-green-300 text-black rounded-lg shadow-lg overflow-hidden cursor-pointer transition-transform transform hover:scale-105"
                style={{ width: '200px', height: 'auto' }} // Smaller size
                onClick={() => openModal(recipe)}
              >
                <img
                  src={recipe.image || 'default-image-url.jpg'}
                  alt={recipe.title}
                  className="w-full h-36 object-cover" // Adjusted height
                />
                <div className="p-2"> {/* Reduced padding */}
                  <h3 className="text-sm font-bold">{recipe.title}</h3> {/* Adjusted text size */}
                  <p className="mt-1 text-xs text-gray-700"> {/* Smaller text */}
                    {recipe.instructions
                      ? `${recipe.instructions.slice(0, 80)}...` // Reduced characters shown
                      : 'No instructions available.'}
                  </p>
                </div>
              </div>
